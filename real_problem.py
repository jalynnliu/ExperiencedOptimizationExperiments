...
def run_for_synthetic_problem():
    ...
    for problem_name in test_10:
        dtrain,dtest = mlbp.get_train_test_data(problem_name)
        opt_error_list = []
...
def run_for_real_problem_ave():
    ...
    for problem_name in test_10:
        ...
        for j in range(opt_repeat):
            ...
            exp_racos.exp_mix_opt(obj_fct=score_fun,ss=sample_size, bud=budget, pn=positive_num, rp=rand_probability, ub=uncertain_bit,
                                     at=adv_threshold)
    ...
    optimization_log_file = result_path + 'opt-log-ave-0.txt'
    print('optimization logging: ', optimization_log_file)
    fo.FileWriter(optimization_log_file, log_buffer, style='w')
